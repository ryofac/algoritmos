...
def maior_numero_vetor(vetor):
    maior_numero = reduzir(vetor, lambda x, y: x if x > y else y)
    return maior_numero

def menor_numero_vetor(vetor):
    menor_numero = reduzir(vetor, lambda x, y: x if x < y else y)
    return menor_numero
...
def escolher_item_aleatorio(vetor):
    if eh_matriz(vetor):
        ...
        return lista_unica[obter_numero_aleatorio_entre(1, obter_tamanho_vetor(lista_unica) - 1)]
    
    return vetor[obter_numero_aleatorio_entre(1, obter_tamanho_vetor(vetor) - 1)]


def obter_numero_aleatorio_entre(min, max):
